...
Prove that if $A$ and $B$ are disjoint denumerable sets, then $A \cup B$ is also denumerable.
...
is bijective by construction, and thus $A \cup B$ is denumerable. $\triangle$
...
Since $f$ and $g$ are onto, it follows that $\text{im}(f)=A$ and $\text{im}(g)=B$, and so $f\left( \frac{n+1}{2} \right) \in A$ and $g\left( \frac{m}{2} \right) \in B$. But $A$ and $B$ are disjoint, so $f\left( \frac{n+1}{2} \right)$ and $g\left( \frac{m}{2} \right)$ cannot be equal, which is a contradiction. This proves the claim that $n$ and $m$ have the same parity.
...
Therefore, $h$ is bijective, which implies that $A\cup B$ is denumerable. $\blacksquare$
